...
bool VF2SubState::IsFeasiblePair(node_id node1, node_id node2)
  { 
    ...
    FILE * fp;
    fp = fopen ("scheiss.log", "a+");
    fprintf (fp, "----- IsFeasiblePair:: %d,%d",node1+1,node2+1);fflush(fp);
        ...
        if (core_1[other1] != NULL_NODE)
          { other2=core_1[other1];
            if (!g2->HasEdge(node2, other2) ||
              ...
              fprintf (fp, " -ret-> 0 (R_succ(subg) failed\n");fflush(fp);
              fclose(fp);
              return false;
            }
          }
        ...
        if (core_1[other1]!=NULL_NODE)
          { other2=core_1[other1];
            if (!g2->HasEdge(other2, node2) ||
              ...
              fprintf (fp, " -ret-> 0 (R_pred(subg) failed)\n");fflush(fp);
              fclose(fp);
              return false;
            }
          }
        ...
        if (core_2[other2]!=NULL_NODE)
          { other1=core_2[other2];
            if (!g1->HasEdge(node1, other1)) {
              fprintf (fp, " -ret-> 0 (R_succ(G) failed)\n");fflush(fp);
              fclose(fp);
              return false;
            }
          }
        ...
        if (core_2[other2] != NULL_NODE)
          { other1=core_2[other2];
            if (!g1->HasEdge(other1, node1)) {
              fprintf (fp, " -ret-> 0 (R_pred(G) failed)\n");fflush(fp);
              fclose(fp);
              return false;
            }
          }
    ...
    bool ret = termin1<=termin2 && termout1<=termout2 && new1<=new2;
    fprintf (fp, " -ret-> %d\n",ret);fflush(fp);
    fclose(fp);
    return ret;
  }
